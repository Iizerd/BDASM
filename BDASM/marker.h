...
{
	if (XED_ICLASS_NOP != xed_decoded_inst_get_iclass(&it->decoded_inst) ||
...
inst_it_t<Addr_width> find_begin_marker(inst_list_t<Addr_width>& list)
{
	...
	uint32_t search_length = list.size() - BDASM_BEGIN_INST_COUNT + 1;
...
inst_it_t<Addr_width> find_end_marker(inst_list_t<Addr_width>& list)
{
	...
	uint32_t search_length = list.size() - BDASM_BEGIN_INST_COUNT + 2;
