...
namespace obf
{
	...
	{
		static std::tuple<dasm::inst_descriptor_t<Addr_width>, dasm::inst_descriptor_t<Addr_width>, bool>
			stack_analysis(dasm::routine_t<Addr_width>& routine)
		{
			dasm::inst_descriptor_t<Addr_width> allocator(routine.blocks.end(), routine.blocks.back().instructions.end()), deallocator = allocator;
			bool bp_based = false;
			for (auto block_it = routine.blocks.begin(); block_it != routine.blocks.end(); ++block_it)
			{
				for (auto inst_it = block_it->instructions.begin(); inst_it != block_it->instructions.end(); ++inst_it)
				{
					...
					if (iform == XED_IFORM_SUB_GPRv_IMMz || iform == XED_IFORM_SUB_GPRv_IMMb &&
						get_max_reg_size<XED_REG_RSP, Addr_width>::value == xed_decoded_inst_get_reg(&inst_it->decoded_inst, XED_OPERAND_REG0/*xed_operand_name(xed_inst_operand(inst, 0))*/))
					{
						allocator.block = block_it;
						allocator.inst = inst_it;
					}
					else if (iform == XED_IFORM_ADD_GPRv_IMMz || iform == XED_IFORM_ADD_GPRv_IMMb &&
						get_max_reg_size<XED_REG_RSP, Addr_width>::value == xed_decoded_inst_get_reg(&inst_it->decoded_inst, XED_OPERAND_REG0 /*xed_operand_name(xed_inst_operand(inst, 0))*/))
					{
						deallocator.block = block_it;
						deallocator.inst = inst_it;
					}
					else if (iform == XED_IFORM_MOV_GPRv_GPRv_89 &&
						get_max_reg_size<XED_REG_RBP, Addr_width>::value == xed_decoded_inst_get_reg(&inst_it->decoded_inst, XED_OPERAND_REG0/*xed_operand_name(xed_inst_operand(inst, 0))*/) &&
						get_max_reg_size<XED_REG_RSP, Addr_width>::value == xed_decoded_inst_get_reg(&inst_it->decoded_inst, XED_OPERAND_REG1/*xed_operand_name(xed_inst_operand(inst, 1))*/)
						)
					{
						bp_based = true;
					}
			...
			return { allocator, deallocator, bp_based };
		}

		// This is pretty rudamentary, it cant handle multiple deallocators or stack changes betwene allocator/deallocator combo
		// allocation_size 
		//	in: the additional size to allocate
		//  out: the original stack allocation size
		//
		static pass_status_t pass_old(context_t<Addr_width>& ctx, dasm::routine_t<Addr_width>& routine, int32_t& allocation_size)
		{
			if (!allocation_size)
				return pass_status_t::failure;

			auto [allocator, deallocator, is_bp_based] = stack_analysis(routine);

			if (allocator.block != routine.entry_block || allocator.block == routine.blocks.end() || deallocator.block == routine.blocks.end())
				return pass_status_t::failure;

			auto original_allocation_size = xed_decoded_inst_get_signed_immediate(&allocator.inst->decoded_inst);
			if (original_allocation_size != xed_decoded_inst_get_signed_immediate(&deallocator.inst->decoded_inst))
				return pass_status_t::failure;

			auto new_allocation_size = original_allocation_size + allocation_size;
			allocation_size = original_allocation_size;

			xed_decoded_inst_set_immediate_signed_bits(&allocator.inst->decoded_inst, new_allocation_size, 32);
			xed_decoded_inst_set_immediate_signed_bits(&deallocator.inst->decoded_inst, new_allocation_size, 32);


			// now loop and edit all displacements
					...
					for (uint32_t i = 0; i < num_operands; ++i)
					{
						...
						if (operand_name == XED_OPERAND_MEM0 || operand_name == XED_OPERAND_AGEN)
						{
								...
								xed_decoded_inst_get_base_reg(&inst_it->decoded_inst, 0))
							{
								...
								//
								if (disp < original_allocation_size)
								{
									xed_decoded_inst_set_memory_displacement_bits(&inst_it->decoded_inst, disp + allocation_size, 32);
								}
							}
							else if (is_bp_based &&
